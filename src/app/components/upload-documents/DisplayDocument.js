...
function DisplayDocument({ index, doc, setIsEditOpen }) {
  const { setSelectedDocumentIdToDelete, setIsDeleteDocumentOpen } =
    useUploadDocumentStore();
  return (
    <div className="ps-8 py-2 border-b-[1px] border-[#C2C6D4] flex flex-col lg:flex-row gap-6 relative">
      <span className="absolute top-4 left-0">{`${index + 1})`}</span>
      <div className="w-full lg:w-2/3 xl:w-3/4 grid grid-cols-1 sm:grid-cols-2 gap-2">
        <p className="sm:col-span-2 h-10 text-sm font-normal leading-6 text-[#191C21] flex items-center">
          {doc.document_description}
        </p>
        <p className="h-10 text-sm font-normal leading-6 text-[#191C21] flex items-center">
        ...
        <p className="row-start-3 h-10 text-sm font-normal leading-6 text-[#191C21] flex items-center">
          <span className="w-2/3 sm:w-1/2 xl:w-1/3 flex items-center gap-2">
            <HiCalendar className="text-[#404753] text-xl sm:text-2xl" />
      ...
      <div className="w-full lg:w-1/3 xl:w-1/4 flex flex-col sm:flex-row gap-6 relative pb-14 sm:pb-0">
        <div className="w-36 md:w-42 lg:w-1/2 xl:w-2/5 h-48 lg:h-full">
